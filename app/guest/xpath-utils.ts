...
(<PPWindow>window).PP_XPATH = (() => {
  ...
  function _getElementsBySelector(doc, css){
    var xpath = _cssToXPath(css);
    return _getElementsByXPath(doc, xpath);
  };

  function _getElementsByXPath(doc, xpath){
    try
    {
      return _evaluateXPath(doc, xpath);
    }
    catch(ex)
    {
      return [];
  ...
  function _getRuleMatchingElements(rule, doc)
  {
  ...
  return {
    ...
    getElementsByXPath: _getElementsByXPath
  }
})();
